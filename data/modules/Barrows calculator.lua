...
function p.main(frame)
	...
	leaguesMultiplier = 1
	if (args.LeaguesMultiplier) then
		leaguesMultiplier = math.max(args.LeaguesMultiplier, 1) --Clamp to 1x multiplier
	end
	...
	local barrowsItemExpectation = leaguesMultiplier * (1.0 * rewardRolls) / (denominator * 4 * #brothersKillList)
	...
	if(rewardRolls == 1) then 
		...
		barrowsLootLine = barrowsLootLine .. "Each roll is a '''1/" ..  denominator / leaguesMultiplier .. "''' chance to be one piece of [[Barrows equipment]]. There are '''" .. 4 * #brothersKillList .. "''' eligible pieces, therefore each is '''1/" .. denominator / leaguesMultiplier * 4 * #brothersKillList .. "''' per roll. On average you will get 1/" .. string.format('%.2f', denominator / leaguesMultiplier /rewardRolls) .. " Barrows items per chest.\n"
