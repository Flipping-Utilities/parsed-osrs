...
local tabOverrides = {
	...
	["All Pets"] = "[[Pet/Collection log|All Pets]]",
	["Skilling Pets"] = "[[Pet#Skilling pets|Skilling Pets]]",
	...
	["Fossil Island Notes"] = "[[Fossil island note book|Fossil Island Notes]]",
	...
	["Ocean Encounters"] = "[[Giant clam|Ocean Encounters]]",
	["Sailing Miscellaneous"] = "[[Sailing/Collection log|Sailing Miscellaneous]]",
	["Sea Treasures"] = "[[Sailing/Collection log|Sea Treasures]]",
	["Slayer"] = "[[Slayer/Collection log|Slayer]]",
}
...
local function getData(category, names)
	local data = {}
...
local function makeTable(data, columns, checkbox)
	local clogTable = mw.html.create('table'):addClass('wikitable lighttable sortable qc-active collection-log autosort=3,d'):cssText('max-width: 800px')
	if not checkbox or checkbox == '' or checkbox == 'show' then
		clogTable = clogTable:attr("data-dynamic-input1", '{"label": "Hide filled log slots", "type": "checkbox", "adjective": "filled", "class": "wikisync-completed"}')
	end
	...
	for _, v in ipairs(data) do
		...
		local rowData = {
			item = string.format("[[File:%s|link=]] %s", image, text),
...
function p.list(frame)
	...
	local names = {}
	local i = 1
	while args['cname'..i] do
		names[i] = args['cname'..i]
		i = i + 1
	end
	return p._list(args['category'], names, args['checkbox'])
end

function p._list(category, names, checkbox)
	local columns = {"item", "collection", "completion"}
	local data = getData(category, names)
	-- table.sort(data, sortByTierAndMonster)
	local rettbl = tostring( makeTable(data, columns, checkbox) )
	return rettbl
end
