...
local combo_runes = {
	['mist rune'] = { ['air rune'] = true, ['water rune'] = true, comboRune = true },
	['dust rune'] = { ['air rune'] = true, ['earth rune'] = true, comboRune = true },
	['mud rune'] = { ['water rune'] = true, ['earth rune'] = true, comboRune = true },
	['smoke rune'] = { ['air rune'] = true, ['fire rune'] = true, comboRune = true },
	['steam rune'] = { ['water rune'] = true, ['fire rune'] = true, comboRune = true },
	['lava rune'] = { ['earth rune'] = true, ['fire rune'] = true, comboRune = true }
}
...
local staves = {
	...
	--]]
	{ name = "slayer's staff", substitutions = {{name = 'staff of the dead'}}, explicit = true},
	{ name = "iban's staff", explicit = true},
	{ name = "saradomin staff", substitutions = {{name = 'staff of light'}}, explicit = true},
	{ name = "guthix staff", substitutions = {{name = 'staff of balance'}}, explicit = true},
	{ name = "zamorak staff", substitutions = {{name = 'staff of the dead', override = {alternatives = {'toxic staff of the dead'}}}}, explicit = true},
	...
	{ name = 'Staff of the Dead', conditions = {args = {{'is_offensive'}}}, alternatives = {'staff of light, staff of balance, or the toxic variant'}, negations = {{magnitude = 0.875}} },
	{ name = 'kodai wand', ['water rune'] = true, conditions = {args = {{'is_offensive'}}}, negations = {{magnitude = 0.85}} },
	
	{ name = 'staff of light', conditions = {args = {{'is_offensive'}}}, negations = {{magnitude = 0.875}}, explicit = true },
	{ name = 'staff of balance', conditions = {args = {{'is_offensive'}}}, negations = {{magnitude = 0.875}}, explicit = true },
	
	--Partial negation should be conditioned upon the rune(s), placed INSIDE the negation parameter
	{ name = "bryophyta's staff", negations = {{conditions = {runes = {'nature rune'}}, magnitude = (14/15)}} }
}
...
function p.create_table(runes, arg_pass)
	local args = arg_pass
	
	if args.req_staff then
		for _, staff in ipairs(staves) do
			if staff.name == args.req_staff then
				args.req_staff = staff
				break
			end
		end
		runes = join({{args.req_staff.name, 0, args.req_staff}},runes)
		mw.logObject(runes)
	end
	
	-- We want to work out weapons first so that we can pass it to combo runes in specific scenarios
	-- add relevant main-hands to the weapons table
	local weapons = {}
	local relevant_staves = {}
	if not args.nostaff or args.nostaff == 0 then
		relevant_staves = composeWeapons(staves, runes, args, args.req_staff)
		weapons = join(weapons, relevant_staves)
	end
	
	-- add relevant off-hands to the weapons table
	local relevant_offhands = {}
	if args.nostaff ~= 1 then
		relevant_offhands = composeWeapons(offhands, runes)
	...
	if #relevant_offhands > 0 and (not args.req_staff or #relevant_combos > 0) then
		ret:tag('tr')
			:tag('th')
				:attr('colspan','2')
			...
			:done()
		:done()
		if not args.req_staff then ret = weapon_output(relevant_offhands,runes,weapons,ret,args) end
...
function composeWeapons(list, runes, args, required)
	local a = {}
	if required then
		local req_item = {}
		...
		table.insert(a, {req_item})
		
		for _, subst in ipairs(req_item.substitutions or {}) do
			local item_copy = {}
			for _, item in ipairs(list) do
				if item.name == subst.name then 
					item_copy = item
					break	
				end
			end
			
			for param, val in pairs(subst) do
				item_copy[param] = val	
			end
			
			table.insert(a, {item_copy})
		end
	else
		for i, v in ipairs(list) do
			if not v.explicit then -- We don't include weapons that have to be explicitly added
				local total = 0
				local rune_count = 0
				...
				if (not a[1][rune_name] and b[1][rune_name]) then
					...
					for ineg, negation in ipairs(a[1].negations or {}) do
						...
						if (not negation.conditions or check_conditions(negation.conditions, args, rune_data[1])) then
							negationMagA = negationMagA * negation.magnitude
						end
					end
					...
					for ineg, negation in ipairs(b[1].negations or {}) do
						-- If the weapon has a negation and either fulfills its conditions OR has no conditions listed, apply it
						if (not negation.conditions or check_conditions(negation.conditions, args, rune_data[1])) then
							negationMagB = negationMagB * negation.magnitude
						end
					end
		...
		for rune_i, rune_data in ipairs(runes) do
			if weapon_data[1][rune_data[1]] or (weapon_data[2] and weapon_data[2][rune_data[1]]) then
				...
				for ineg, negation in ipairs(negations) do
					-- If the weapon has a negation and either fulfills its conditions OR has no conditions listed, apply it
					if (not negation.conditions or check_conditions(negation.conditions, args, rune_data[1])) then
						negationMag = negationMag * negation.magnitude
					end
				end
				
				if negationMag == 0 then
					-- Do nothing
				elseif negationMag < 1 then
					table.insert(tbl, { rune_data[1], round(rune_data[2] * negationMag,2), {} })
				else
...
function make_pics(arg, others)
	...
	for _, v in ipairs(runes) do
		...
		if #altNext ~= 0 then
			...
			table.insert(ret,"<sub class='explain' title='Alternatively, a "..alts.." can be used.'>Alt</sub>")
		end
	end
