...
function p.reweight(raid_level)
	-- temporary working implemetation of unique reweighting (summer sweep-up 2025) until someone knowledgeable makes it better / optimizes it
...
function p.get_rewards(points, raid_level, team_size, leagues_harder_mode)
	-- adjusted raid level for purple chance calcs
	...
	-- adjusted raid level for pet chance calcs
	local adjusted_raid_level_pet = raid_level
	if adjusted_raid_level_pet > 400 then
		if adjusted_raid_level_pet > 550 then
			adjusted_raid_level_pet = 450
		else
			adjusted_raid_level_pet = 400 + math.floor((adjusted_raid_level_pet - 400) / 3)
		end
	end
	...
	if leagues_harder_mode then -- leagues none sense
		...
		pet_denominator = 100 * (350000 - 700 * math.min(adjusted_raid_level_pet, 466)) -- we dont know if this is correct
	else
		purple_denominator = math.max(purple_denominator, 150000) -- caps at 1500
		pet_denominator = 100 * (350000 - 700 * adjusted_raid_level_pet)
	end
...
[[With no deaths, this raid will give approximately '''%s''' contribution points%s. With a raid level of '''%s''', this equates to a '''%.2f%%''' chance of receiving a purple drop%s, and a '''%.2f%%''' chance of receiving the pet%s.
