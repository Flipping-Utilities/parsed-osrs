...
-- separate step to process the data with overrides and stuff
local function processData(data, columns)
	local processed = {}
	...
	for _, v in ipairs(data) do
		...
		table.insert(processed, { row = row, raw = rowData })
	end

	return processed
end

local function makeTable(processedItems, columns)
	...
	for _, item in ipairs(processedItems) do
		...
		local htmlRow = clogTable:tag('tr'):attr('data-item-id', raw._id)
		if raw._isClueJunk then
...
function p._list(args)
	...
	local category = args['category'] ~= nil and args['category'] or 'all'
	local data = getData(args['category'])

	-- separate step to process the data with overrides and stuff
	local processedItems = processData(data, columns)

	local rettbl
	-- if outputType arg is not specified or is explicitly set to 'table'
	if (args['outputType'] ~= nil and args['outputType'] == 'raw') then
		-- return a raw lua table to be used in other modules
		return processedItems
	else
		-- return processed html string for templates
		return tostring( makeTable(processedItems, columns) )
	end
end
