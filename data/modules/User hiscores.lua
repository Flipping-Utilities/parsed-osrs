...
function p._main(raw_args)
	...
	local stats_table = make_frame(main_tr)
		local tr = stats_table:tr()
			:th()
				...
				:tag('span')
					:addClass('plainlinks')
					:wikitext(string.format('[https://secure.runescape.com/m=hiscore_oldschool/hiscorepersonal.ws?user1=%s %s]', mw.uri.encode(name), name))
				:done()
		make_spacer(stats_table:tr(), 3)
			...
			tr = stats_table:tr()
				...
				make_cell(tr:td(), 'sailing', 'Sailing icon')
		...
		make_spacer(stats_table:tr(), 3)
			...
			tr = total_stats_table:tr()
				...
				make_cell(tr:td(), 'collections logged', 'Collection log', 'Collection log')
		tr:done()
	stats_table:done()
	...
	make_spacer(right_side_table:tr(), 3)
		local bosses_tr = right_side_table:tr()
			...
			tr = make_frame(left_bosses_table:tr(), 6)
				make_cell(tr:td(), 'brutus', 'beef')
			...
			tr = make_frame(left_bosses_table:tr(), 6)
				...
				make_cell(tr:td(), 'the royal titans', 'Bran')
			...
			tr = make_frame(left_bosses_table:tr(), 6)
				...
				make_cell(tr:td(), 'mad angel', 'Aggy')
			tr = make_frame(left_bosses_table:tr(), 6)
				...
				make_cell(tr:td(), 'yama', 'Yami')
			tr = make_frame(left_bosses_table:tr(), 6)
				make_cell(tr:td(), 'doom of mokhaiotl', 'Dom', 'Doom of Mokhaiotl')
			...
			tr = make_frame(left_bosses_table:tr(), 6)
				...
				make_cell(tr:td(), 'maggot king', 'Maggot marquess')
		...
		make_spacer(right_bosses_table, 4)
			tr = make_frame(right_bosses_table:tr(), 4)
				...
				make_cell(tr:td(), 'shellbane gryphon', 'Gull (pet)')
			tr = right_bosses_table:tr()
