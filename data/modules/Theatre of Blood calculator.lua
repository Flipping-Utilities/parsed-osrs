...
local uniques_hm = {
	{name = 'Avernic defender hilt', chance = 7},
	{name = 'Ghrazi rapier', chance = 2},
	{name = 'Sanguinesti staff (uncharged)', chance = 2},
	{name = 'Justiciar faceguard', chance = 2},
	{name = 'Justiciar chestguard', chance = 2},
...
function p.calc(frame)
	...
	local hard   = (args.mode == 'Hard')
	local U      = hard and uniques_hm      or uniques
	local Utotal = hard and uniquestotal_hm or uniquestotal
	local urate  = hard and 7.7 or 9.1
	local qmult  = 1
	if hard then qmult = (args.timebonus == 'true') and 1.30 or 1.15 end
	...
	if(args.splitdrops == 'true') then
		...
		local dropchance = 1/urate*ratio
		if(teampoints == 0) then
			...
			for _, item in ipairs(noobies) do
				ret[#ret+1] = frame:expandTemplate{title='DropsLine', args = {Name=item.name,Quantity=1,Rarity='Always',bucket='no'}}
			end
		...
		for _, item in ipairs(U) do
			...
			local chance = dropchance*item.chance/Utotal/args.teammates
			local quantity = 1
		...
		for _, item in ipairs(normies) do
			...
			local quantity = (math.floor(item.min*qmult*ratio)+math.floor(math.floor(item.min*qmult*ratio)*1.2))/2
		...
		ret[#ret+1] = frame:expandTemplate{title='DropsTableHead'}
		for _, item in ipairs(U) do
			ret[#ret+1] = frame:expandTemplate{title='DropsLine', args = {Name=item.name,Quantity=1,Rarity=mw.ustring.format('1/%.3f',Utotal/item.chance/dropchance),bucket='no',Rolls=1/args.teammates}}
		end
		for _, item in ipairs(normies) do
			ret[#ret+1] = frame:expandTemplate{title='DropsLine', args = {Name=item.name,Quantity=mw.ustring.format('%i-%i',item.min*qmult*ratio,math.floor(item.min*qmult*ratio)*1.2),Rarity=mw.ustring.format('%i/%i',item.chance,normiestotal),bucket='no',Rolls=3}}
		end
		ret[#ret+1] = frame:expandTemplate{title='DropsTableBottom'}
		return table.concat(ret,'')
	end
	...
	for _, item in ipairs(U) do
		local baseprice = geprice(item.name)
		local chance = dropchancesolo*item.chance/Utotal
		local quantity = 1
		uniqueprice = uniqueprice + baseprice*chance*quantity
	end
	for _, item in ipairs(normies) do
	...
	ret[#ret+1] = frame:expandTemplate{title='DropsTableHead'}
	for _, item in ipairs(U) do
		ret[#ret+1] = frame:expandTemplate{title='DropsLine', args = {Name=item.name,Quantity=1,Rarity=mw.ustring.format('1/%i',Utotal/item.chance/dropchancesolo),bucket='no'}}
	end
	for _, item in ipairs(normies) do
		ret[#ret+1] = frame:expandTemplate{title='DropsLine', args = {Name=item.name,Quantity=mw.ustring.format('%i-%i',item.min*qmult*ratio,math.floor(item.min*qmult*ratio)*1.2),Rarity=mw.ustring.format('%i/%i',item.chance,normiestotal),bucket='no',Rolls=3}}
	end
