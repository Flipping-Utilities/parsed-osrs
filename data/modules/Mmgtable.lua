...
function p._mmgtable(frame, args)
	...
	if set_bucket then
		...
		for _, output in ipairs(parsedOutput.list) do
			local qty_per_hour = output.qty

			if is_per_kill and not output.isph then
				qty_per_hour = output.qty * (tonumber(args.kph) or 1)
			end

			bucket("mmg_output").put({
				item = output.name,
