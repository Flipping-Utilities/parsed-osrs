...
function p._main(points, skills, hasPhoenix, magicEligible, torstolEligible, sellPyro)
	...
	if magicEligible then
		...
		uniquesTable:node(makeLine("Warm gloves", 1, 1, 1/150 * totalProbability, rolls, pyroPrice))
	...
	if torstolEligible then
		...
		uniquesTable:node(makeLine("Bruma torch", 1, 1, 1/150 * totalProbability, rolls, pyroPrice))
		uniquesExpectation = uniquesExpectation + 1/150 * totalProbability * rolls * pyroPrice
	end
	totalProbability = totalProbability * (1 - 1/150)
	...
	for skill, skillLevel in pairs(skills) do
		...
		for _, item in ipairs(items[skill]) do
			...
			local price = 0
			if item.name ~= 'Spirit seed' then
				price = geprice(item.name)
			end
			local expectedCount = chancePerRoll * rolls * 3/24 * (item.minQty + item.maxQty) / 2
			resourcesExpectation = resourcesExpectation + expectedCount * price
			
			local row = makeLine(item.name, item.minQty, item.maxQty, chancePerRoll * 3/25, rolls, price)
			resourcesTable:node(row)
		end
	end
	...
	for _, item in ipairs(items.other) do
		...
		local row = makeLine(item.name, item.minQty, item.maxQty, item.chance, rolls, item.price)
		resourcesTable:node(row)
	end

	resourcesTable:tag('tr')
		:tag('th')
			:attr('colspan', 6)
			:css('text-align','right')
			:wikitext('Total:')
