...
local attributes = {
	...
	flying = '[[Flying (attribute)|Flying]]',
...
function p._main(args)
	...
	ret:defineParams{
		{ name = 'default_version', func = { name = is_default_version, params = { 'smwname'..default_version_index, 'version'..default_version_index, 'smwname', 'version' } } },
		{ name = 'name', func = 'name'},
		...
		{ name = 'elementalweaknesspercent', func = { name = elementalweaknesspercentarg, params = {'elementalweaknesspercent', 'elementalweaknesstype'}, flag = {'p', 'p'}} },
		...
		{ name = 'immunecannon_smw', func = { name = immunearg_smw, params = {'immunecannon', 'immunecannon'}, flag = { 'p', 'r' } } },
		{ name = 'immunethrall', func = { name = immunearg, params = {'immunethrall', 'immunethrall'}, flag = { 'd', 'r', 'r' } } },
		{ name = 'immunethrall_smw', func = { name = immunearg_smw, params = {'immunethrall', 'immunethrall'}, flag = { 'p', 'r' } } },
		...
		{ name = 'chisel_link', func = { name = 'chisel_link', params = { 'id', 'name', 'npc' }, flag = { 'p', 'p', 'r' } } },
	...
	ret:useBucket("infobox_monster", {
		members = 'is_members_only',
		release = 'release_date',
		id_smw = 'id',
		image_smw = 'image',
		combat_smw = 'combat_level',
		examine = 'examine',
		poisonous = 'poisonous',
		attributes_smw = 'attribute',
		hitpoints_smw = 'hitpoints',
		max_hit_smw = 'max_hit',
		slaylvl_smw = 'slayer_level',
		slayxp_smw = 'slayer_experience',
		usesskill = 'uses_skill',
		assignedby_smw = 'assigned_by',
		att_smw = 'attack_level',
		str_smw = 'strength_level',
		def_smw = 'defence_level',
		range_smw = 'ranged_level',
		mage_smw = 'magic_level',
		amagic_smw = 'magic_attack_bonus',
		arange_smw = 'range_attack_bonus',
		dstab_smw = 'stab_defence_bonus',
		dslash_smw = 'slash_defence_bonus',
		dcrush_smw = 'crush_defence_bonus',
		dmagic_smw = 'magic_defence_bonus',
		drange_smw = 'range_defence_bonus', -- TODO: Remove this after light/standard/heavy are used
		dlight_smw = 'light_range_defence_bonus',
		dstandard_smw = 'standard_range_defence_bonus',
		dheavy_smw = 'heavy_range_defence_bonus',
		attbns_smw = 'attack_bonus',
		strbns_smw = 'strength_bonus',
		rngbns_smw = 'range_strength_bonus',
		mbns_smw = 'magic_damage_bonus',
		version = 'version_anchor',
		name = 'name',
		cat_smw = 'slayer_category',
		immunepoison_smw = 'poison_immune',
		immunevenom_smw = 'venom_immune',
		immunethrall_smw = "thrall_immune",
		immunecannon_smw = "cannon_immune",
		immuneburn = 'burn_immune',
		['attack style_smw'] = 'attack_style',
		['attack speed_smw'] = 'attack_speed',
		xpbonus_smw = 'experience_bonus',
		flatarmour_smw = 'flat_armour',
		size_smw = 'size',
		freezeresistance = 'freeze_resistance',
		elementalweaknesstype_smw = 'elemental_weakness',
		elementalweaknesspercent_smw = 'elemental_weakness_percent',
		leagueRegion_raw = 'league_region',
		default_version = 'default_version',
	})

	ret:useBucket('npc_id', {
			id_smw = 'id'
	})
	...
	if ret:paramDefined('leagueRegion', 'all') then
		ret:addRow{
			{ tag = 'th', content = '[[Raging Echoes League|League]] region', colspan = '8' },
			{ tag = 'argd', content = 'leagueRegion', colspan = '16' },
	...
	:addRow{
		{ tag = 'th', content = '[[File:Attack icon.png|link=Attack#Attack_bonuses|Monster attack bonus]]', colspan = '4', class = 'infobox-nested' },
	...
	:addRow{
		{ tag = 'th', content = '[[File:Magic icon.png|link=Magic]]', colspan = '24', class = 'infobox-nested' }
	}
	:addRow{
		{ tag = 'argd', content = 'dmagic', colspan = '24', class = 'infobox-nested' }
	}
	...
	-- A "nice" 1-1 mapping for now between attributes and weakness. Expecting exceptions and more logic in the future. (Loar shade gadderhammer ignore?).
	local demonbane_weapons = ret:paramGrep('attributes', function(val) return val:find("Demon") ~= nil end)
	local dragonbane_weapons = ret:paramGrep('attributes', function(val) return val:find("Draconic") ~= nil end)
	local golembane_weapons = ret:paramGrep('attributes', function(val) return val:find("Golem") ~= nil end)
	local vampyrebane_weapons = ret:paramGrep('attributes', function(val) return val:find("Vampyre") ~= nil end)
	local keris_weapons = ret:paramGrep('attributes', function(val) return val:find("Kalphite") ~= nil end)
	local rat_bone_weapons = ret:paramGrep('attributes', function(val) return val:find("Rat") ~= nil end)
	local leaf_bladed_battleaxe = ret:paramGrep('attributes', function(val) return val:find("Leafy") ~= nil end)
	local gadderhammer = ret:paramGrep('attributes', function(val) return val:find("Shade") ~= nil end)
	local salve_amulets = ret:paramGrep('attributes', function(val) return val:find("Undead") ~= nil end)
	local sea_curse = ret:paramGrep('attributes', function(val) return val:find("Fiery") ~= nil end)

	local weaknesses = {demonbane_weapons, dragonbane_weapons, golembane_weapons, vampyrebane_weapons, keris_weapons, rat_bone_weapons, leaf_bladed_battleaxe, gadderhammer, salve_amulets, sea_curse}
	local weaknesses_count = 1 -- Starts at 1 because we always have elemental weakness shown, even without elemental weakness.
	for _, v in ipairs(weaknesses) do
		if v then weaknesses_count = weaknesses_count + 1
		end
	end

	-- Determine column span based on the amount of weaknesses.
	local weakness_colspan = 24/weaknesses_count

	ret:pad(24)
	:addRow{
		{ tag = 'th', content = '[[File:Weaken.png]] Weaknesses', colspan = '24', class = 'infobox-subheader' }
	}
	:pad(24)

	-- Always add elemental weakness
	local weakness_headers = {{ tag = 'argh', content = 'elementalweaknesstype', colspan = weakness_colspan, class = 'infobox-nested' }}
	local weakness_values = {{ tag = 'argd', content = 'elementalweaknesspercent', colspan = weakness_colspan, class = 'infobox-nested' }}

	if demonbane_weapons
	then
    	table.insert(weakness_headers,{ tag = 'th', content = '[[File:Silverlight.png|link=Demonbane weapons|Demonbane weapons]]', colspan = weakness_colspan, class = 'infobox-nested' })
		table.insert(weakness_values,{ tag = 'td', content = 'Demonbane weapons', colspan = weakness_colspan, class = 'infobox-nested' })
	end

	if dragonbane_weapons then
    	table.insert(weakness_headers,{ tag = 'th', content = '[[File:Dragon hunter crossbow.png|link=Dragonbane weapons|Dragonbane weapons]]', colspan = weakness_colspan, class = 'infobox-nested' })
		table.insert(weakness_values,{ tag = 'td', content = 'Dragonbane weapons', colspan = weakness_colspan, class = 'infobox-nested' })
	end

	if golembane_weapons then
    	table.insert(weakness_headers,{ tag = 'th', content = '[[File:Barronite mace.png|link=Golembane weapons|Golembane weapons]]', colspan = weakness_colspan, class = 'infobox-nested' })
		table.insert(weakness_values,{ tag = 'td', content = 'Golembane weapons', colspan = weakness_colspan, class = 'infobox-nested' })
	end

	if vampyrebane_weapons then
    	table.insert(weakness_headers,{ tag = 'th', content = '[[File:Rod of Ivandis.png|link=Monster_attribute|Vampyrebane weapons]]', colspan = weakness_colspan, class = 'infobox-nested' })
		table.insert(weakness_values,{ tag = 'td', content = 'Vampyrebane weapons', colspan = weakness_colspan, class = 'infobox-nested' })
	end

	if keris_weapons then
    	table.insert(weakness_headers,{ tag = 'th', content = '[[File:Keris.png|link=Monster_attribute|Keris weapons]]', colspan = weakness_colspan, class = 'infobox-nested' })
		table.insert(weakness_values,{ tag = 'td', content = 'Keris weapons', colspan = weakness_colspan, class = 'infobox-nested' })
	end

	if rat_bone_weapons then
    	table.insert(weakness_headers,{ tag = 'th', content = '[[File:Bone mace.png|link=Rat bone weapons|Rat bone weapons]]', colspan = weakness_colspan, class = 'infobox-nested' })
		table.insert(weakness_values,{ tag = 'td', content = 'Rat bone weapons', colspan = weakness_colspan, class = 'infobox-nested' })
	end
	
	if salve_amulets then
    	table.insert(weakness_headers,{ tag = 'th', content = '[[File:Salve amulet.png|link=Salve_amulet#Comparison|Salve amulets]]', colspan = weakness_colspan, class = 'infobox-nested' })
		table.insert(weakness_values,{ tag = 'td', content = 'Salve amulets', colspan = weakness_colspan, class = 'infobox-nested' })
	end

	if sea_curse then
    	table.insert(weakness_headers,{ tag = 'th', content = '[[File:Pearl_dragon_bolts_(e)_5.png|link=Pearl_dragon_bolts_(e)#Special_effect|Sea curse]]', colspan = weakness_colspan, class = 'infobox-nested' })
		table.insert(weakness_values,{ tag = 'td', content = 'Sea curse', colspan = weakness_colspan, class = 'infobox-nested' })
	end

	if leaf_bladed_battleaxe then
    	table.insert(weakness_headers,{ tag = 'th', content = '[[File:Leaf-bladed_battleaxe.png|link=Leaf-bladed_battleaxe|Leaf-bladed battleaxe]]', colspan = weakness_colspan, class = 'infobox-nested' })
		table.insert(weakness_values,{ tag = 'td', content = 'Leaf-bladed battleaxe', colspan = weakness_colspan, class = 'infobox-nested' })
	end

	if gadderhammer then
    	table.insert(weakness_headers,{ tag = 'th', content = '[[File:Gadderhammer.png|link=Gadderhammer|Gadderhammer]]', colspan = weakness_colspan, class = 'infobox-nested' })
		table.insert(weakness_values,{ tag = 'td', content = 'Gadderhammer', colspan = weakness_colspan, class = 'infobox-nested' })
	end

	ret:addRow(weakness_headers)
	ret:addRow(weakness_values)
	...
	:addRow{
		{ tag = 'th', content = 'Link', colspan = '8' },
		{ tag = 'argd', content = 'chisel_link',  colspan = '16' },
		meta = {addClass = 'advanced-data'}
	}
	:pad(24, 'advanced-data')
...
function elementalweaknesspercentarg(percentage, elementaltype)
	if not infobox.isDefined(percentage) or string.lower(percentage) == 'no' then
		return 'No elemental weakness'
	end

	return '+' .. percentage .. '% ' .. elementaltype .. ' spells'
end
...
function usedpscalcarg(arg, id)
	...
	local firstid = ids[1]
	return '<div class="dps-calc plainlinks">[https://tools.runescape.wiki/osrs-dps/?monster=' .. firstid .. ' <span class="cdx-button cdx-button--action-progressive cdx-button--weight-primary dps-calc-openbtn" style="min-height:0; margin-bottom: 8px; display: inline-flex; align-items: center; gap: 4px; padding:5px 12px">[[File:Dps calc logo.png|32px|link=]] Open in DPS calculator</span>]</div>'
end
...
function is_default_version(default_smwVer, default_textVer, smwVer, textVer)
	if default_smwVer ~= '' then
		local tmp = default_smwVer == smwVer
		return tmp
	end
...
function addcategories(args, catargs)
	...
	local cat_map = {
		...
		matches = {
			...
			immunepoison = { immune = 'Monsters immune to poison', ['not immune'] = 'Monsters susceptible to poison' },
			immunevenom = { immune = 'Monsters immune to venom', ['not immune'] = 'Monsters susceptible to venom' },
		}
	}
