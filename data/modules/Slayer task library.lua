...
local tableNames = {"Turael", "Krystilia", "Mazchna", "Vannaka", "Chaeldar", "Konar", "Nieve", "Duradel", "Mortimer"}
...
function p.get_effective_table(tableToReduce, status, returnDisallowed)
	...
	for taskMonsterId, originalTaskValue in pairs(tableToReduce) do
		if check_requirements(
			status,
			originalTaskValue['requirements'],
			taskMonsterId
		) then
			-- Always copy the task so conditional changes do not modify
			-- the original master table.
			local taskValue = {}

			for k, v in pairs(originalTaskValue) do
				taskValue[k] = v
			end

			-- Apply an alternate weight when the associated unlock is active.
			local unlockWeight = taskValue['unlockWeight']

			if unlockWeight ~= nil then
				local unlockName =
					SlayerConsts.get_unlock_name(unlockWeight['unlock'])

				if unlockName ~= nil
					and status['Unlocks'][unlockName] == true
				then
					taskValue['weight'] = unlockWeight['weight']
				end
			end
			...
			if subtable ~= nil then
				local effectiveSubtable, unavailableSubtable =
					p.get_effective_table(
						subtable,
						status,
						returnDisallowed
					)
