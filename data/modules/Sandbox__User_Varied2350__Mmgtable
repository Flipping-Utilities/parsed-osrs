...
function expr(x)
	local e_g, e = pcall(mw.ext.ParserFunctions.expr, string.gsub(x, ',', ''))
	if e_g then
		return e
	end
	return nil
...
function handleBucketLines(args, MAX_INPUTS)
	local drop_lines = bucket('dropsline')
		.select('item_name', 'drop_json')
		.where('page_name', args['pageName'])
		.limit(100)
		.offset(0)
		.run()
		
	-- local decoded = mw.text.jsonDecode(drop_lines[1])
	local table_lines = {}
	local items  ={}
	local total_item_value = 0
	local value_per_kill = 0
	...
	for k, v in ipairs(drop_lines) do
		local drop_line = mw.text.jsonDecode(v['drop_json'])
		local min_quant = drop_line["Quantity Low"]
		local max_quant = drop_line["Quantity High"]
		local avg_quant = (min_quant + max_quant) / 2
		local rarity = drop_line['Rarity']
		local altRarity = drop_line['Alt Rarity']
		if altRarity ~= "" then
			rarity = altRarity
		end
		mw.log('Item: ' .. v['item_name'] .. ', min: ' .. min_quant .. ', max: ' .. max_quant .. ', rarity: ' .. rarity)
		mw.log('avg_quant: ' .. avg_quant)
		mw.log('rarity expr: ' .. expr(rarity))
		avg_quant = avg_quant * expr(rarity)
		
		local span = mw.html.create('span')
		span:addClass('mmg-itemline mmg-output mmg-varieswithkph')
		
		local success, price = pcall(gePrice, v['item_name'])
		price = success and tonumber(price)
		price = price or 0
		
		price = deduct_tax(v['item_name'], price)
		
		local total_value = tonumber(price) * tonumber(avg_quant)
		value_per_kill = value_per_kill + total_value
		span:tag('span'):addClass('mmg-quantity'):attr('data-mmg-qty', avg_quant):wikitext(autoround(avg_quant, true))
		span:wikitext(string.format(' x [[File:%s.png|link=%s]] [[%s]] (', v['item_name'], v['item_name'], v['item_name']))
		span:tag('span'):addClass('mmg-cost'):attr('data-mmg-cost-pk', total_value):wikitext(_coins(autoround(price), 'nocoins'))
		span:wikitext(')')
		
		table.insert(table_lines,  span)
		table.insert(items, {name = v['item_name'], qty = avg_quant, value = total_value, isph = true, pricetype = 'gemw' })
	end
	
	return {value = value_per_kill, valuepk = value_per_kill, valueph = value_per_kill, spans = table_lines, list = items}
end
...
function handleOutputs(args)
	if yn(args['useBucket']) then
		return handleBucketLines(args, MAX_INPUTS)
	else
