...
function p.main(frame)
	...
	-- kephri_heal_percent: 0-200, percentage of a full shield's HP dealt across both regen cycles.
	-- 100% = one full shield killed with no regen; 200% = both shields fully regenerate; default 150%.
	local kephri_heal_percent = math.max(0, math.min(200, tonumber(args.kephri_heal_percent) or 150))

	out = p.calculator(raid_level, team_size, path_invocation, walk_the_path, leagues_harder_mode, three_down, kephri_heal_percent, skulls_skipped)
	mw.log(out)
	return out
end
...
--    kephri_heal_percent (number) – 0-200, % of a full shield's HP dealt
-- ============================================================
function p.estimate_points(raid_level, team_size, path_invocation, walk_the_path, three_down, kephri_heal_percent, skulls_skipped)
	...
	--   One full shield = 250 HP. kephri_heal_percent scales linearly from that base:
	--   0%   = shields killed instantly, no HP dealt (theoretical floor)
	--   100% = one full shield killed, no regen
	--   150% = original default assumption (partial regen across two cycles)
	--   200% = both shields fully regenerate and get fully killed twice
	local kephri_shield_base_hp = 250 * kephri_heal_percent / 100
	local kephri_shield_approx_note = true
...
-- ============================================================
--  calculator  (new params: three_down, kephri_heal_percent)
-- ============================================================
function p.calculator(raid_level, team_size, path_invocation, walk_the_path, leagues_harder_mode, three_down, kephri_heal_percent, skulls_skipped)
	skulls_skipped = skulls_skipped or 0
	-- default new params to original behaviour if not supplied
	if three_down == nil then three_down = false end
	if kephri_heal_percent == nil then kephri_heal_percent = 150 end
	...
	local points_breakdown = p.estimate_points(raid_level, team_size, path_invocation, walk_the_path, three_down, kephri_heal_percent, skulls_skipped)
	...
	if kephri_heal_percent ~= 150 then
		table.insert(toggle_notes, string.format("* '''Kephri shield healing''' set to '''%d%%''' of a full shield's HP (100%% = one full shield with no regen, 200%% = both shields fully regenerate).", kephri_heal_percent))
	end
...
* Higher or lower healing from Kephri's shields than estimated
