...
import { convertMaterialsToObject, parseRecipeProperties } from "./recipes.extractor";
...
describe("parseRecipeProperties", () => {
  it("parses the real A stone bowl recipe template", () => {
    ...
    expect(recipe!.notes).toBe("Requires partial completion of Elemental Workshop I");
  ...
  it("parses all Bronze bar recipe templates with skills, ticks, and multi-skill", () => {
    ...
    const furnaceRecipe = parsedRecipes.find((recipe) => recipe.facility === "Furnace");
    ...
    expect(furnaceRecipe!.ticksNote).toBe("Make-X is 4, then 5. A single bar is 6.");
    ...
    const blastRecipe = parsedRecipes.find((recipe) => recipe.facility === "Blast Furnace");
    ...
    const superheatRecipe = parsedRecipes.find((recipe) => recipe.skills.length === 2);
    ...
    expect(superheatRecipe!.inputs.map((input) => input.id)).toEqual([436, 438, 561, 554]);
...
describe("convertMaterialsToObject", () => {
  it("converts real A stone bowl recipe inputs and outputs", () => {
    ...
    const outputs = convertMaterialsToObject(stoneBowlRecipe, "output", itemLookup);
