...
import { parseNewsFromContent, parseUpdateTemplate, renderNewsBody } from "./news.extractor";
...
describe("parseNewsFromContent", () => {
  it("parses a newspost into a NewsArticle with metadata and body", () => {
    const page = loadTestPage(TestPages.SuperStrengthBeerCheat);
    const article = parseNewsFromContent(page.text, page.title, page.aliases, page.id);
    ...
    expect(article!.wikiUrl).toBe("https://oldschool.runescape.wiki/w/?curid=47397");
    expect(article!.body).toContain("super strong");
  });

  it("returns null for a page with no Update template", () => {
    const page = loadTestPage(TestPages.StoneBowl);
    expect(parseNewsFromContent(page.text, page.title, page.aliases, page.id)).toBeNull();
  });
});
