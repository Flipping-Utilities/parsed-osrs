...
describe('parseQuestFromContent', () => {
  it("parses Cook's Assistant details, recommended items and required-for", () => {
    ...
    expect(quest!.itemRequirements).toEqual(
      expect.arrayContaining(['Pot', 'Bucket'])
    );
    ...
    expect(quest!.rewards).toEqual(
      expect.arrayContaining([
        'Cooking 300 experience',
        expect.stringContaining('Cook-o-matic'),
      ])
    );
    ...
    expect(quest!.requiredFor).toEqual(
      expect.arrayContaining([
        'Recipe for Disaster',
        expect.stringContaining('Easy Lumbridge'),
      ])
    );
    ...
    expect(headings).toEqual(
      expect.arrayContaining(['A feast for a Duke', 'Delivery'])
    );
  ...
  it("parses Legends' Quest skill requirements, kills, and ironman concerns", () => {
    ...
    expect(quest!.requiredFor).toEqual(
      expect.arrayContaining([
        'Dragon Slayer II',
        expect.stringContaining('Recipe for Disaster'),
      ])
    );
...
describe('parseQuickGuideFromContent', () => {
  it("parses Legends' Quest /Quick guide into ordered step groups with checklists", () => {
    ...
    expect(
      guide.every((g) => g.steps.every((s) => !/Chat option/i.test(s)))
    ).toBe(true);
  });
...
describe('buildQuickGuideTitle', () => {
  it('appends the conventional /Quick guide suffix', () => {
    expect(buildQuickGuideTitle("Cook's Assistant")).toBe(
      "Cook's Assistant/Quick guide"
    );
    expect(buildQuickGuideTitle("Legends' Quest")).toBe(
