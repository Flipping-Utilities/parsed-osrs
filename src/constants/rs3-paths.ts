...
/**
 * Output paths for the RS3 pipeline.
 *
 * Mirrors `src/constants/paths.ts` (the OSRS layout) but rooted under
 * `DATA_FOLDER_PATH_RS3` (default `./data/rs3`) and `WIKI_FOLDER_PATH_RS3`
 * (default `./wiki-data-rs3`). OSRS paths stay untouched — RS3 has its own
 * directory tree so the two datasets never collide on disk.
 */
export const DATA_FOLDER = process.env.DATA_FOLDER_PATH_RS3 || "./data/rs3";
export const WIKI_DATA_FOLDER = process.env.WIKI_FOLDER_PATH_RS3 || "./wiki-data-rs3";

// Raw wiki content lives on its own path (separate repo / larger volume)
